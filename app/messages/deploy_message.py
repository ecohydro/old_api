...
class DeployMessage(Message):
    ...
    def nbk_id(self):
        if self._created and self.content:
            return str(hashlib.sha224(str(self._created) +
                       str(self.content)).hexdigest()[:10])
        else:
            self.status = 'invalid'
            assert 0, "Uh-oh. No message nbk_id."
    ...
    def parse(self):
        ...
        try:
            for j in range(self.n_sensors()):
                ...
                self.data['sensors'].append(
                    s[self.config['ITEM_LOOKUP_FIELD']])
                i += 2
        except:
            self.status = 'invalid'
        ...
        # Do the GOOGLE Stuff:
        location = google_geolocate_api(
            self.data['cellTowers'],
            self.config['GOOGLE_API_KEY'])
        self.data['location'] = location if location \
            else self.config['LOCATION']
        elevation = google_elevation_api(
            self.data['location'],
            self.config['GOOGLE_API_KEY'])
        self.data['elevation'] = elevation if elevation \
            else self.config['ELEVATION']
        self.data['address'] = google_geocoding_api(
            self.data['location'],
            self.config['GOOGLE_API_KEY'])
